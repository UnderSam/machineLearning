...
def mf_sgd(R, K=64, alpha=1e-4, beta=1e-2, iterations=50):
    ...
    for iters in range(iterations):
        ...
        for i, j, r in samples:
            ...
            """
            rhead = b + b_u[i] + b_i[j] + np.dot(P[i],Q[j])
            dist = r - rhead
            b_u[i] = b_u[i] - alpha*((-1*dist)+(beta*b_u[i]))
            b_i[j] = b_i[j] - alpha*((-1*dist)+(beta*b_i[j]))
            P[i] = P[i] - alpha*((-1*dist*Q[j])+(beta*P[i]))
            Q[j] = Q[j] - alpha*((-1*dist*P[i])+(beta*Q[j]))
